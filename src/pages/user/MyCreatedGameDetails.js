...
import { ArrowLeft, Calendar, Clock3, MapPin, Trophy, Users , GamepadIcon} from "lucide-react";
...
export default function MyCreatedGameDetails() {
  ...
  useEffect(() => {
    const fetchCreatedGameDetails = async () => {
      try {
        setLoading(true);
    ...
    return (
      <div className="p-0">
        <div className="space-y-4">
          <button
            type="button"
            onClick={() => navigate("/profile/mygames")}
      ...
      <div className="space-y-6">
        ...
        <div className={`bg-gradient-to-br border rounded-2xl p-6 md:p-8 ${
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
            <h1 className={`text-2xl sm:text-3xl font-bold break-words ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{game?.name || "Game"}</h1>
