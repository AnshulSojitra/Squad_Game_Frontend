...
export default function Bookingslot() {
  ...
  return (
    <>
      <div
        className={`min-h-screen px-4 sm:px-6 lg:px-8 py-16 sm:py-20 transition-colors duration-300 ${
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          <p
            className={`text-center mb-6 sm:mb-8 text-sm sm:text-base max-w-2xl mx-auto ${
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2">
            <div className="text-gray-400 text-sm">
              {loading ? "Loading grounds..." : `${filteredGrounds.length} ground${filteredGrounds.length !== 1 ? "s" : ""} found`}
            </div>
          ...
          {loading ? (
            ...
            <ShowMore
              ...
              initialCount={ITEMS_PER_BATCH}
              increment={ITEMS_PER_BATCH}
              containerClassName="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-8"
              renderItem={(ground, index) => (
                <div
                  key={ground.id}
